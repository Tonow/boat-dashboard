...
if len(date_to_unpack) == 1:
    with st.spinner("En attente 📅 de la date de fin"):
        sleep(2)


else:
    start_time, end_time = date_to_unpack

    st.subheader(f"Du {start_time} au {end_time}")
    filtered_data = data[(data.index > pd.to_datetime(start_time)) & (data.index <= pd.to_datetime(end_time + timedelta(days=1)))]
